...
\begin{longtable}{ | p{4cm} | p{5cm} | p{5cm} | p{5cm} |}
	...
	&
	\textbf{\citeA{anderson:2007}}: the general ACT-R framework has, in its later versions, been explicitly linked to fMRI brain research. The framework assumes distinct cognitive modules and claims they can be related to respective parts of the brain.
	\\
    \hline
...
well-explained by the DDM. Importantly, parametical changes in projection
strengths in the neurocomputational model (those between the subthalamic
nucleus and globus pallidus, interal segment) were reflected by corresponding
...
localization assumptions represent linking hypotheses, and subsequent studies
...
which localize moudles. These predictions were confirmed by
...
In recent work, these predictions have become more quantitative and can be
...
\begin{subfigure}{0.85\textwidth}
  ...
  \caption{\emph{The traditional GLM-approach} 
	\\\hspace{\textwidth} The traditional GLM approach is tailored to
	find differences in BOLD activation between experimental conditions.
	To do this, a hypothetical neural signal is constructed for every
	...
	condition in that particular voxel. The GLM also yields the variance
	of those estimates. By subtracting beta-values and combining their
...
\begin{subfigure}[b]{0.85\textwidth}
  ...
  Firstly, for some cognitive models it is infeasible to get single-trial
  estimates of a particular parameter. The prime example of this are evidence
  accumulation-models, which are fitted to an entire distribution of
...
behavior during tasks involving the learning of values associated with
...
of symbolic reasoning models, which was inspired by its succeses in
reinforcement learning research. Models constructed in recent versions of the
...
that BOLD activity in the pre-SMA and the doral ACC correlated significantly
...
a dataset where students solved linear algebra problems in a MRI scanner,
...
neuroimaging data, it is possible to reliably predict in which state of
...
different substeps of solving an algebra problem were not made explicit in
...
experimental conditions. For example, RT differences across difficulty levels
...
assumed to modulate only the amplitude of the task-locked hemodynamic
...
mapping  \cite{OReilly:2011ca}. This is so because many other hypotheses can
